...
directorResource_ <- R6::R6Class("directorResource",
  ...
  public = list(
    ...
    evaluate = function(source_args, args = list()) {
      route <- director$match_preprocessor(name)
    ...
    parse = function(value, provides, args = list()) {
      ...
      route <- director$match_parser(name)
    ...
    show = function() {
      cat("Resource", sQuote(name), "under director: \n")
      director$show()
    },
    ...
    get_dependencies = function() {
      get_dependencies_ <- function(key) {
      ...
      unique(c(recursive = TRUE, as.character(cached$dependencies),
        sapply(cached$dependencies, get_dependencies_)))
    },

    # TODO: (RK) Test this method!
    dependencies_modified = function() {
      dependency_resources <- lapply(get_dependencies(), director$resource, soft = TRUE)
    ...
    caching_enabled = function() {
      any_is_substring_of(name, director$cached_resources())
    },
