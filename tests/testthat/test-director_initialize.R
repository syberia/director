...
describe("invalid inputs", {
  ...
  test_that("it errors if project name is not scalar", {
    expect_error(director$new(tempdir(), character(0)), "must be a scalar")
    expect_error(director$new(tempdir(), c("foo", "bar")), "must be a scalar")
  })
})
